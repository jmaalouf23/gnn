...
class MainModel_1(torch.nn.Module): #Takes one molecule as input
    ...
    def __init__(self, hyper):
        ...
        mlp_input_size = self.hyper.hidden_size 
        
        self.mlp = nn.Sequential( nn.Linear(mlp_input_size, self.hyper.ffn_hidden_size), 
                                  nn.ReLU(),
                                  nn.Linear( self.hyper.ffn_hidden_size, self.hyper.ffn_hidden_size), 
                                  nn.ReLU(),  
                                  nn.Linear(self.hyper.ffn_hidden_size, 1))
    ...
    def forward(self, x):
        ...
        # # Pass into MLP 
        output = self.mlp(vec_solute)

        return output

class MainModel_1_hyperopt(torch.nn.Module): #Takes one molecule as input
    """Contains a message passing networks (MPN) for representing a molecule...
    followed by an MLP readout layer for predicting solubility."""
    ...
    def forward(self, x):
        ...
        # If augmenting default atom features with custom atom features
        if self.hyper.atom_descriptors == 'feature':
            #Accourding to chemprop documentation, this is only supported for one molecule
            custom_atom_features_solute = [ [x[i][1]] for i in range(len(x)) ] # A list of numpy arrays (N_atoms x N_features) containing custom atom features
            #custom_atom_features_solvent = x[3] # A list of numpy arrays (N_atoms x N_features) containing custom atom features

        # Bond features (use chemprop default)
        bond_features = (None,)
        # Apply MPNs to obtain solute vector representations
        vec_solute = self.mpn_solute(smiles_list_solute, atom_features_batch = custom_atom_features_solute, bond_features_batch=bond_features)
        ...
        # Concatenate solute and solvent vectors
        # Pass into MLP 
        output = self.ffn(vec_solute)

        return output
    
class MainModel_2(torch.nn.Module):
    """Contains two separate message passing networks (MPN) for representing solute and solvent...
    followed by an MLP readout layer for predicting solubility."""
    ...
    #Main model class
class MainModel_1_old(torch.nn.Module):
    """Contains two separate message passing networks (MPN) for representing solute and solvent...
    followed by an MLP readout layer for predicting solubility."""
    ...
    def __init__(self, hyper):
        ...
        # Define MLP readout layer (one hidden layer for now)
        mlp_input_size = self.hyper.hidden_size # input to MLP is concatenated solute and solvent representations
        self.mlp = nn.Sequential( nn.Linear(mlp_input_size, self.hyper.mlp_hidden_size), 
                                  nn.ReLU(),
                                  nn.Linear( self.hyper.mlp_hidden_size, self.hyper.mlp_hidden_size), 
                                  nn.ReLU(),  
