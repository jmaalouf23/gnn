...
def train(rank, world_size, hostname,args):
    ...
    hyper.atom_descriptors = None # when using default atom features set to 'features'
    ...
    logger.info(f'num_folds = {args.n_fold}')
    ...
    #We will train num_folds random splits for the number of epochs specified below.
    ...
    for i in range(num_folds):
        ...
        os.makedirs(f'{mydrive}/Models/{ModelFolder}/fold_{i}',exist_ok=True)
        ...
        for epoch in range(epochs):
            ...
            if val_loss == min(val_losses):
                ...
                torch.save(model.state_dict(), f'{mydrive}/Models/{ModelFolder}/fold_{i}/best_model.pt')
        ...
        for data in train_loader:
            ...
            y_train_pred = np.append(y_train_pred, y_pred.cpu().detach().numpy()) 
            y_train_all = np.append(y_train_all, np.squeeze(y_train,axis=1))       
        

        for data in test_loader:
            x_test, y_test = data
            y_pred = model(x_test)
            y_test_pred = np.append(y_test_pred, y_pred.cpu().detach().numpy())
            y_test_all = np.append(y_test_all, np.squeeze(y_test,axis=1))
        ...
        make_learning_curve(epochs,train_losses, val_losses, f'{mydrive}/Models/{ModelFolder}/fold_{i}/epochcurve_{i}.png')
        make_parity(y_train_pred,y_train_all,y_test_pred,y_test_all,f'{mydrive}/Models/{ModelFolder}/fold_{i}/parity_{i}.png',mse_train, mse_test)
        
        model_train_losses.append(train_losses);
        model_val_losses.append(val_losses);
        
        np.save(f'{mydrive}/Models/{ModelFolder}/fold_{i}/y_train_true.npy',y_train_all)
        np.save(f'{mydrive}/Models/{ModelFolder}/fold_{i}/y_train_pred.npy',y_train_pred)
        np.save(f'{mydrive}/Models/{ModelFolder}/fold_{i}/y_test_true.npy',y_test_all)
        np.save(f'{mydrive}/Models/{ModelFolder}/fold_{i}/y_test_pred.npy',y_test_pred)
    
    #Plot epoch curve
    model_train_losses_arr=np.array(model_train_losses) #2D array of Num models by Num epochs
